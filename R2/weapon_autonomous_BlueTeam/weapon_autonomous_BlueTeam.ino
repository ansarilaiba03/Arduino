...
// =====================================
long leftCounts    = 730;   // strafe left
long forwardCounts = 1500;   // move forward
...
void loop()
{
      ...
      readIMU();   // update currentYaw from Serial3
  ...
  // =====================================
// STATE 1
// MOVE FORWARD + CHECK PROXIMITY
// =====================================

else if(state == 1)
{
  // Check sensor continuously while moving
  if(digitalRead(proxPin) == LOW)
  {
    ...
    Serial.println("SPEARHEAD FOUND DURING FORWARD MOTION");

    state = 4;   // go directly to gripper sequence
  }
  ...
  else
  {
    ...
    state = 2;   // continue with existing logic
  }
}
